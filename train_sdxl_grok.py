...
while step < total_steps:
    ...
    for _ in range(gradient_accumulation_steps):
        ...
        text_embeddings = batch['text_embeddings'].to(device, dtype=torch.float16)
        text_embeds = batch['text_embeds'].to(device, dtype=torch.float16)
        time_ids = batch['time_ids'].to(device, dtype=torch.float16)
